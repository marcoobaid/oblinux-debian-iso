...
timestamped log under `build-logs/`, validates the branding pin, prepares the
...
1. Validate the Brand Master integration and build its pinned local Debian
   package from the checksum-verified immutable source archive.
2. Download the pinned Papirus input and build the local OBLinux icon package.
3. Bootstrap a minimal Trixie filesystem.
4. Configure Debian package repositories inside the chroot.
5. Install the kernel, live components, GNOME, firmware, selected apps, and the
   local OBLinux branding and icon packages.
6. Apply configured hooks and included OBLinux files.
7. Verify Brand Master payload/activation and both Horizon themes, their
   caches, aliases, package status, and GNOME
   default.
8. Remove temporary package data as appropriate.
9. Compress the live filesystem into SquashFS.
10. Assemble GRUB boot files and the hybrid ISO.
11. Generate checksums and package/file metadata.
